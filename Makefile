...
build-docker-image: ## Builds the production x86_64 Docker image
	docker build . --no-cache --file Dockerfile --tag phanan/koel-dev:latest
...
clear-cache: ## Clear caches that sometimes cause error 500
	docker exec --user www-data -it koeldev php artisan cache:clear

see-logs: ## Tail -f laravel logs
	docker exec --user www-data -it koeldev tail -f storage/logs/laravel.log
...
start: ## Build and start the DEV docker-compose stack
	...
	@echo "Done! Koel should now be available at http://localhost ⛳"
